...
> 提醒：這關核心就是「沒加 `-a` 看不到隱藏檔」。如果這裡只下 `ls`（沒 `-a`），目錄看起來會像空的，`...Hiding-From-You` 完全不會出現 — 養成永遠 `ls -al` 的習慣就不會被坑。
>
> 另一個保險寫法：`cat ./...Hiding-From-You`，明確告訴 shell「這是當前目錄的檔名」，避免被誤判（**跟 Level 1→2 的處理方式一樣**）。
